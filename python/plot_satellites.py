...
def plot_satellites(csv_file, max_epochs=1000, output_file=None):
    ...
    if len(unique_times) > max_epochs:
        # Take only the first max_epochs time points
        selected_times = unique_times[:max_epochs]
        valid_data = valid_data[np.isin(valid_data[:, 0], selected_times)]
        print(
            f"Limited to first {max_epochs} epochs ({len(valid_data)} data points) for performance"
        )

    # Separate data by satellite and ensure time ordering
    ...
    # Plot each satellite with time-ordered data
    ...
    for i, sat in enumerate(satellites):
        ...
        if len(sat_data) > 0:
            ...
            # Check if we have enough consecutive data points for a trajectory
            if len(sat_data) > 10:  # Only plot if we have sufficient data points
                ax.plot(
                    sat_data[:, 2],
                    sat_data[:, 3],
                    sat_data[:, 4],
                ...
                # Plot as scatter points for satellites with limited data
                ax.scatter(
                    sat_data[:, 2],
                    sat_data[:, 3],
                    sat_data[:, 4],
